...
- `apps/backend` — NestJS + TypeScript. `AuthModule`, `UsersModule`, `WalletsModule`, and `TransactionsModule` have real logic; `LedgerModule`, `OutboxModule`, `MessagingModule` are still empty *Nest module* skeletons even though their Prisma tables already exist. `PrismaModule` and `CacheModule` (Redis via `RedisService`) are `@Global()` — available everywhere without importing them explicitly.
- `POST /transactions/transfer` requires an `Idempotency-Key` header (UUID). Concurrency safety currently comes from Postgres optimistic locking (`wallets.version` column, checked inside a `prisma.$transaction`) — this already prevents lost updates under real concurrent requests, independent of the Redis distributed lock planned for Scope 6 (which is there to cut down on retries/latency under contention, not the sole correctness mechanism).
